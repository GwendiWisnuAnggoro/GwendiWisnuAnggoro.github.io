...
function SistemBestScore(score){
    let BestScore = 0;
    if(!localStorage.getItem("Best2")){
        ctx.fillText('Score Tertinggi: ' + localStorage.setItem("Best2", "0"), canvas.width-350, 15);
        
    } else{
        ctx.fillText('Score Tertinggi: ' + localStorage.getItem("Best2"), canvas.width-350, 15);

    }
    if(BestScore !== null){
        if(score > localStorage.getItem("Best2")){
            ctx.fillText('Score Tertinggi: ' + localStorage.setItem("Best2", score), canvas.width-350, 15);
        }
    }else{
        ctx.fillText('Score Tertinggi: ' + localStorage.setItem("Best2", score), canvas.width-350, 15);
    }
...
document.getElementById("MulaiPC").addEventListener("click", ()=>{
    playerReset();
    updateScore();
    update();
    document.getElementById("MulaiPC").style.display = "none";
})
draw();
